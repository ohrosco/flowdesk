...
export async function POST(req) {
  ...
  const { data: lead, error: leadErr } = await db
    .from("leads")
    .insert({ name, phone, email, address, service, source, timeline, notes, status: "new", stage: "New Lead" })
    .select()
    .single();
  ...
  if (smsBody && process.env.TWILIO_ACCOUNT_SID) {
    try {
      ...
      await db.from("followup_messages").insert({
        lead_id: lead.id, step: 0, channel: "sms",
  ...
  const scheduled = sequence.map(({ step, channel, delayHours }) => {
    ...
    return { lead_id: lead.id, step, channel, status: "pending", scheduled_at: scheduledAt.toISOString() };
  });
